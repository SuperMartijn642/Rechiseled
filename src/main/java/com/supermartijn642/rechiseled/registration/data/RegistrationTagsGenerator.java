...
public class RegistrationTagsGenerator extends TagGenerator {
    ...
    static{
        ...
        }, (name, title, required, resources, metadata, position, source) ->
            new Pack(name, title, required, resources, metadata, PackType.SERVER_DATA, position, source));
    ...
    private Set<ResourceLocation> getTagsForBlock(Block block){
        return Stream.of(
    ...
    private List<Block> loadVanillaTag(ResourceLocation location){
        ...
        try{
            for(Resource resource : this.resources.getResources(tagLocation)){
            ...
            throw new RuntimeException("Encountered an exception whilst iteration resources for tag '" + tagLocation + "':", e);
        }
