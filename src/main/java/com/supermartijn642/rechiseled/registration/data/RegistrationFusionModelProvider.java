...
public class RegistrationFusionModelProvider extends FusionModelProvider {
    ...
    private static ConnectionPredicate isBlock(Block block){
        if(block.getBlockState().getProperties().contains(RechiseledPillarBlock.AXIS_PROPERTY))
            ...
            return DefaultConnectionPredicates.matchState(block, Pair.of(RechiseledPillarBlock.AXIS_PROPERTY, EnumFacing.Axis.Y));
    ...
    private static ConnectionPredicate isTopStairs(Block stairs){
        //noinspection unchecked,SuspiciousNameCombination
        return stairs == null ? DefaultConnectionPredicates.or() : DefaultConnectionPredicates.matchState(stairs, Pair.of(RechiseledStairBlock.HALF, BlockStairs.EnumHalf.TOP));
    }

    private static ConnectionPredicate isBottomStairs(Block stairs){
        //noinspection unchecked,SuspiciousNameCombination
        return stairs == null ? DefaultConnectionPredicates.or() : DefaultConnectionPredicates.matchState(stairs, Pair.of(RechiseledStairBlock.HALF, BlockStairs.EnumHalf.BOTTOM));
    }
