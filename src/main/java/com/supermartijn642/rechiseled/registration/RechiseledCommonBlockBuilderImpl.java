...
public abstract class RechiseledCommonBlockBuilderImpl<T extends RechiseledCommonBlockBuilder<?>> implements RechiseledCommonBlockBuilder<T> {
    ...
    void createBlocks(BlockSpecification specification){
        ...
        if(this.hasConnectingVariant && this.parent.hasConnectingVariant)
            handler.registerBlock(this.identifier + "_connecting", () -> {
                ...
                connectingBlockHolder.set(this.createBlock(specification, parent, true, propertiesHolder.get(), new ResourceLocation(blockIdentifier.getNamespace(), blockIdentifier.getPath() + "_connecting")));
    ...
    @Override
    public T itemGroups(ItemGroup group, ItemGroup... groups){
        this.checkMutable();
        List<ItemGroup> tabs = new ArrayList<>(Arrays.asList(groups));
        tabs.add(group);
