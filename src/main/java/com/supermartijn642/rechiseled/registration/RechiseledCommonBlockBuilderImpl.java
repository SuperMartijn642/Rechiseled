...
public abstract class RechiseledCommonBlockBuilderImpl<T extends RechiseledCommonBlockBuilder<?>> implements RechiseledCommonBlockBuilder<T> {
    ...
    private int customRegularVariantMeta, customConnectingVariantMeta;
    private Set<CreativeTabs> itemGroups;
    ...
    protected Supplier<ItemBlock> regularItem, connectingItem;
    ...
    public ItemWithMeta getRegularItem(){
        if(this.customRegularVariant != null)
            return ItemWithMeta.of(this.customRegularVariant.get(), this.customRegularVariantMeta);
        return this.hasRegularVariant ? ItemWithMeta.of(this.regularItem.get()) : null;
    }
    ...
    public ItemWithMeta getConnectingItem(){
        if(this.customConnectingVariant != null)
            return ItemWithMeta.of(this.customConnectingVariant.get(), this.customConnectingVariantMeta);
        return this.hasConnectingVariant ? ItemWithMeta.of(this.connectingItem.get()) : null;
    }
    ...
    void createBlocks(BlockSpecification specification){
        ...
        Holder<BlockProperties> propertiesHolder = new Holder<BlockProperties>() {
            ...
            public BlockProperties get(){
                ...
                if(value == null){
                    value = RechiseledCommonBlockBuilderImpl.this.properties.get();
        ...
        if(this.hasConnectingVariant && this.parent.hasConnectingVariant)
            handler.registerBlock(this.identifier + "_connecting", () -> {
                ...
                connectingBlockHolder.set(this.createBlock(specification, parent, true, propertiesHolder.get(), new ResourceLocation(blockIdentifier.getResourceDomain(), blockIdentifier.getResourcePath() + "_connecting")));
                return connectingBlockHolder.get();
            });

        // Create holders to put the items in once they have been registered.
        Holder<ItemBlock> regularItemHolder = new Holder<>();
        Holder<ItemBlock> connectingItemHolder = new Holder<>();
    ...
    @Override
    public T itemGroups(CreativeTabs group, CreativeTabs... groups){
        this.checkMutable();
        List<CreativeTabs> tabs = new ArrayList<>(Arrays.asList(groups));
        tabs.add(group);
        this.itemGroups = ImmutableSet.copyOf(tabs);
        return this.self();
    }
    ...
    @Override
    public T regularVariant(Supplier<Block> block, int meta){
        this.noRegularVariant();
        this.customRegularVariant = block;
        this.customRegularVariantMeta = meta;
        return this.self();
    }

    @Override
    public T regularVariant(Supplier<Block> block){
        return this.regularVariant(block, 0);
    }

    @Override
    public T connectingVariant(Supplier<Block> block, int meta){
        this.noConnectingVariant();
        this.customConnectingVariant = block;
        this.customConnectingVariantMeta = meta;
        return this.self();
    }

    @Override
    public T connectingVariant(Supplier<Block> block){
        return this.connectingVariant(block, 0);
    }

    @Override
    public T blockTag(String namespace, String identifier){
        this.checkMutable();
        this.blockTags.add(new ResourceLocation(namespace, identifier));
        return this.self();
    }
    ...
    public T itemTag(String namespace, String identifier){
        this.checkMutable();
        this.itemTags.add(new ResourceLocation(namespace, identifier));
        return this.self();
    }
