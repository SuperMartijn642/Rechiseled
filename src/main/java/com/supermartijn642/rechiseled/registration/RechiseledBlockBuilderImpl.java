...
public class RechiseledBlockBuilderImpl extends RechiseledCommonBlockBuilderImpl<RechiseledBlockBuilder> implements RechiseledBlockBuilder {
    ...
    @Override
    public RechiseledBlockBuilder regularVariant(Supplier<Block> block, int blockMeta, Supplier<Block> stairs, int stairsMeta, Supplier<Block> slab, int slabMeta){
        this.checkMutable();
        this.regularVariant(block, blockMeta);
        if(stairs != null)
            this.withStairs(b -> b.regularVariant(stairs, stairsMeta));
        if(slab != null)
            this.withSlabs(b -> b.regularVariant(slab, slabMeta));
        return this;
    }

    @Override
    public RechiseledBlockBuilder regularVariant(Supplier<Block> block, Supplier<Block> stairs, Supplier<Block> slab){
        return this.regularVariant(block, 0, stairs, 0, slab, 0);
    }

    @Override
    public RechiseledBlockBuilder connectingVariant(Supplier<Block> block, int blockMeta, Supplier<Block> stairs, int stairsMeta, Supplier<Block> slab, int slabMeta){
        this.checkMutable();
        this.connectingVariant(block, blockMeta);
        if(stairs != null)
            this.withStairs(b -> b.connectingVariant(stairs, stairsMeta));
        if(slab != null)
            this.withSlabs(b -> b.connectingVariant(slab, slabMeta));
        return this;
    }

    @Override
    public RechiseledBlockBuilder connectingVariant(Supplier<Block> block, Supplier<Block> stairs, Supplier<Block> slab){
        return this.connectingVariant(block, 0, stairs, 0, slab, 0);
    }
