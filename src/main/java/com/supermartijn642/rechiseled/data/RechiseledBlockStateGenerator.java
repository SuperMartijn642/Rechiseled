...
public class RechiseledBlockStateGenerator extends BlockStateGenerator {
    ...
    public void generate(){
        Registries.BLOCKS.getEntries().stream()
            .filter(pair -> pair.left().getResourceDomain().equals("rechiseled"))
            .forEach(pair -> this.blockState(pair.right()).emptyVariant(builder -> builder.model("block/" + pair.left().getResourcePath())));
    }
}
