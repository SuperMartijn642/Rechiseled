...
public class REIScrollableSlotsWidget extends WidgetWithBounds implements DraggableStackProviderWidget {
    ...
    private static final Identifier SCROLLBAR_BACKGROUND = Rechiseled.identifier("textures/screen/rei_scrollbar_background.png");
    ...
    public @Nullable Tooltip getTooltip(TooltipContext context){
        return this.getSlotMouseInteraction(
            ...
            (slot, relativeMouseX, relativeMouseY) ->
                slot.getTooltip(TooltipContext.of(context.getPoint(), context.vanillaContext(), context.getFlag(), context.isSearch())),
            null
        );
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick){
        if(this.scrollable && event.button() == 0 && event.x() > this.x + BOX_WIDTH + 2 && event.x() < this.x + WIDTH && event.y() > this.y && event.y() < this.y + HEIGHT){
            this.dragging = true;
            return true;
        }
        return this.getSlotMouseInteraction(
            (int)event.x(), (int)event.y(),
            (slot, relativeMouseX, relativeMouseY) -> slot.mouseClicked(new MouseButtonEvent(relativeMouseX, relativeMouseY, event.buttonInfo()), doubleClick),
            false
        );
    }
    ...
    public boolean mouseReleased(MouseButtonEvent event){
        ...
        return this.getSlotMouseInteraction(
            (int)event.x(), (int)event.y(),
            (slot, relativeMouseX, relativeMouseY) -> slot.mouseReleased(new MouseButtonEvent(relativeMouseX, relativeMouseY, event.buttonInfo())),
            false
        );
    }
    ...
    public boolean mouseDragged(MouseButtonEvent event, double deltaX, double deltaY){
        ...
        return this.getSlotMouseInteraction(
            (int)event.x(), (int)event.y(),
            (slot, relativeMouseX, relativeMouseY) -> slot.mouseDragged(new MouseButtonEvent(relativeMouseX, relativeMouseY, event.buttonInfo()), deltaX, deltaY),
            false
        );
    }
