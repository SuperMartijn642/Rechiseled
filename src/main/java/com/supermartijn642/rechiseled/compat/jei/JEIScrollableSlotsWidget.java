...
public class JEIScrollableSlotsWidget {
    ...
    private void tryScrollToTarget(){
        float scrollerPosition = Math.clamp(this.targetPosition, this.scrollerPosition - this.scrollerSpeed, this.scrollerPosition + this.scrollerSpeed);
        ...
        ScreenUtils.withScissor(pose, 0, 0, BOX_WIDTH, HEIGHT, () -> {
            ...
            for(int row = 0; row < ROWS + 1; row++){
                for(int column = 0; column < COLUMNS; column++){
                    ...
                    if(index >= this.slots.size())
                        this.slotBackground.draw(graphics, column * SLOT_WIDTH + 1, row * SLOT_HEIGHT - (int)((rowOffset % 1) * SLOT_HEIGHT) + 1);
                }
            }
        });
