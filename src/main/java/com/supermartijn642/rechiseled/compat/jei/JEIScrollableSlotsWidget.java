...
public class JEIScrollableSlotsWidget {
    ...
    public JEIScrollableSlotsWidget(int x, int y, List<JEIScrollableSlotsSlot> slots){
        this.x = x;
        this.y = y;
    ...
    private void setScrollTarget(float targetPosition){
        this.targetPosition = MathHelper.clamp(targetPosition, 0, 1);
    }

    private void tryScrollToTarget(){
        float scrollerPosition = MathHelper.clamp(this.targetPosition, this.scrollerPosition - this.scrollerSpeed, this.scrollerPosition + this.scrollerSpeed);
    ...
    public void draw(double mouseX, double mouseY){
        ...
        ScreenUtils.drawTexture(BOX_WIDTH + 2, 0, SCROLLBAR_WIDTH, HEIGHT);
        // Render cursor
        if(this.scrollable){
            ...
            ScreenUtils.drawTexture(BOX_WIDTH + 3, y, 12, SCROLLER_HEIGHT, 0, highlighted ? 1 / 3f : 0, 1, 1 / 3f);
        }else{
            ScreenUtils.bindTexture(SCROLLER);
            ScreenUtils.drawTexture(BOX_WIDTH + 3, 1, SCROLLBAR_WIDTH - 2, SCROLLER_HEIGHT, 0, 2 / 3f, 1, 1 / 3f);
        }

        // Render missing slots
        ScreenUtils.bindTexture(BOX_BACKGROUND);
        ScreenUtils.drawTexture(0, 0, BOX_WIDTH, HEIGHT);
