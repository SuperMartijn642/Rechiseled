...
public class JEIScrollableSlotsWidget {
    ...
    private final List<Holder<JEIScrollableSlotsSlot>> slots;
    ...
    public JEIScrollableSlotsWidget(int x, int y, List<Holder<JEIScrollableSlotsSlot>> slots, IDrawable slotBackground){
        this.x = x;
        this.y = y;
        ...
        for(int row = 0; row < this.rows; row++){
            for(int column = 0; column < COLUMNS; column++){
                ...
                if(index >= this.slots.size())
                    break loop;
                this.slots.get(index).get().updatePosition(
                    this.x + column * SLOT_WIDTH + 1,
                    this.y + row * SLOT_HEIGHT - (int)(rowOffset * SLOT_HEIGHT) + 1,
                    this.x, this.y,
                    BOX_WIDTH, HEIGHT
                );
            }
        ...
        ScreenUtils.withScissor(poseStack, 0, 0, BOX_WIDTH, HEIGHT, () -> {
            ...
            for(int row = 0; row < ROWS + 1; row++){
                for(int column = 0; column < COLUMNS; column++){
                    ...
                    if(index >= this.slots.size())
                        this.slotBackground.draw(poseStack, column * SLOT_WIDTH + 1, row * SLOT_HEIGHT - (int)((rowOffset % 1) * SLOT_HEIGHT) + 1);
                }
            }
        });
