...
public class ChiselingRecipeCategory implements IRecipeCategory<ChiselingRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder b, ChiselingRecipe recipe, IFocusGroup focuses){
        if(!(b instanceof RecipeLayoutBuilder)){
            IRecipeSlotBuilder input = b.addSlot(RecipeIngredientRole.INPUT, 0, 0);
            IRecipeSlotBuilder output = b.addSlot(RecipeIngredientRole.OUTPUT, 0, 0);
            for(ChiselingBlockShape shape : ChiselingBlockShape.values()){
                for(ChiselingEntry entry : recipe.entries()){
                    if(entry.hasRegularItem(shape)){
                        ...
                        input.addItemStack(new ItemStack(entry.getRegularItem(shape).item()));
                        //noinspection DataFlowIssue
                        output.addItemStack(new ItemStack(entry.getRegularItem(shape).item()));
                    }
                    if(entry.hasConnectingItem(shape)){
                        //noinspection DataFlowIssue
                        input.addItemStack(new ItemStack(entry.getConnectingItem(shape).item()));
                        //noinspection DataFlowIssue
                        output.addItemStack(new ItemStack(entry.getConnectingItem(shape).item()));
                    }
                }
            }
            return;
        ...
        IIngredientManager ingredientManager = JEIFieldAccess.getRecipeLayoutBuilderIngredientManager(recipeLayoutBuilder);
        List<RecipeSlotBuilder> slots = JEIFieldAccess.getRecipeLayoutBuilderSlots(recipeLayoutBuilder);
        ...
        // Input slot
        RecipeSlotBuilder input = new JEIScrollableSlotsSlot.Builder(ingredientManager, JEIFieldAccess.getRecipeLayoutBuilderNextSlotIndex(recipeLayoutBuilder), RecipeIngredientRole.INPUT, 73, 1);
        JEIFieldAccess.setRecipeLayoutBuilderNextSlotIndex(recipeLayoutBuilder, JEIFieldAccess.getRecipeLayoutBuilderNextSlotIndex(recipeLayoutBuilder) + 1);
        input.addTooltipCallback(tooltip);
        inputs.forEach(item -> input.addItemStack(new ItemStack(item)));
        if(inputs.stream().anyMatch(focusedInputs::contains))
            input.setBackground(this.focussedSlot, 0, 0);
        else
            input.setBackground(this.slot, 0, 0);
        slots.add(input);

        // Output slots
        List<Holder<JEIScrollableSlotsSlot>> outputSlots = new ArrayList<>();
        for(List<Item> items : outputs){
            Holder<JEIScrollableSlotsSlot> slotHolder = new Holder<>();
            RecipeSlotBuilder output = new JEIScrollableSlotsSlot.Builder(ingredientManager, JEIFieldAccess.getRecipeLayoutBuilderNextSlotIndex(recipeLayoutBuilder), RecipeIngredientRole.OUTPUT, slotHolder::set);
            JEIFieldAccess.setRecipeLayoutBuilderNextSlotIndex(recipeLayoutBuilder, JEIFieldAccess.getRecipeLayoutBuilderNextSlotIndex(recipeLayoutBuilder) + 1);
            output.addTooltipCallback(tooltip);
            items.forEach(item -> output.addItemStack(new ItemStack(item)));
            if(items.stream().anyMatch(focusedOutputs::contains))
                output.setBackground(this.focussedSlot, 0, 0);
            else
                output.setBackground(this.slot, 0, 0);
            slots.add(output);
            outputSlots.add(slotHolder);
        }
    ...
    public boolean mousePressed(ChiselingRecipe recipe, double mouseX, double mouseY, int button){
        JEIScrollableSlotsWidget scrollableSlotsWidget = (JEIScrollableSlotsWidget)((ChiselingRecipeImpl)recipe).jeiScrollBox;
