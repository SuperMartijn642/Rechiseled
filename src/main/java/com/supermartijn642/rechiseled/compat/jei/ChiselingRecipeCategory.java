...
public class ChiselingRecipeCategory implements IRecipeCategory<ChiselingRecipe> {
    ...
    public String getTitle(){
        return TextComponents.fromTextComponent(this.getTitleAsTextComponent()).format();
    }
    ...
    public void setRecipe(IRecipeLayout recipeLayout, ChiselingRecipe recipe, IIngredients ingredients){
        ...
        ITooltipCallback<ItemStack> tooltip = (slot, input, stack, builder) -> {
            ...
            if(worth != 1){
                ...
                builder.add(TextComponents.translation("rechiseled.recipe_category.conversion_value", TextComponents.number(worth).color(TextFormatting.GOLD).get()).color(TextFormatting.GRAY).get());
            }
        };
    ...
    @Override
    public void draw(ChiselingRecipe recipe, MatrixStack poseStack, double mouseX, double mouseY){
        // Arrow
