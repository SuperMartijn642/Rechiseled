...
public class ShapeSelectionWidget extends AbstractButtonWidget {
    ...
    protected void getTooltips(Consumer<ITextComponent> tooltips){
        ...
        if(display != null && display.entry().hasShape(this.shape))
            tooltips.accept(TextComponents.translation("rechiseled.chiseling.select_shape", TextComponents.fromTextComponent(this.shape.translation()).color(TextFormatting.GOLD).get()).get());
    }
