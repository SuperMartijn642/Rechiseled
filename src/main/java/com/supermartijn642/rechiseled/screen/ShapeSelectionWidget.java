...
public class ShapeSelectionWidget extends AbstractButtonWidget {
    ...
    public void renderBackground(WidgetRenderContext context, int mouseX, int mouseY){
        ...
        ScreenUtils.drawTexture(SMALL_GREY_BUTTONS, context.poseStack(), this.x, this.y, this.width, this.height, 0, (canClick ? this.isFocused() ? 2 : 0 : 1) / 3f, 1, 1 / 3f);
    }
    ...
    public void render(WidgetRenderContext context, int mouseX, int mouseY){
        DisplayEntry display = this.currentEntry.get();
        if(display != null && display.entry().hasShape(this.shape))
            ScreenUtils.drawTexture(this.icon, context.poseStack(), this.x + 1, this.y + 2, this.width - 2, this.height - 4);
    }
}
