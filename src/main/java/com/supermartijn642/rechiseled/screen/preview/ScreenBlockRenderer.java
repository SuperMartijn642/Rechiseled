...
public class ScreenBlockRenderer {
    ...
    public static void drawBlock(PoseStack poseStack, MultiBufferSource.BufferSource bufferSource, BlockCapture capture, double x, double y, double scale, float yaw, float pitch, boolean flatShading){
        ...
        BlockVertexConsumerProvider blockBufferSource = layer -> bufferSource.getBuffer(layer == ChunkSectionLayer.TRANSLUCENT ? Sheets.translucentBlockItemSheet() : Sheets.cutoutBlockSheet());
    ...
    private static void renderBlock(BlockAndTintGetter level, BlockPos pos, BlockState state, PoseStack poseStack, BlockVertexConsumerProvider bufferSource){
        poseStack.pushPose();
        ...
        blockRenderer.getModelRenderer().render(level, model, state, pos, poseStack, bufferSource, true, seed, OverlayTexture.NO_OVERLAY);
