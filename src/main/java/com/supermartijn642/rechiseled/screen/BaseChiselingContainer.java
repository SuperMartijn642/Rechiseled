    ...
    public void chiselAll(boolean includeAllShapes){
        ...
        for(int index = 0; index < inventory.getContainerSize(); index++){
            ...
            if(stack.isEmpty() || stack.getItem() == targetItem || !this.currentRecipe.contains(stack.getItem()))
                continue;
            if(!stack.getComponentsPatch().isEmpty()) // Safety check to prevent overwriting important items
                continue;
            ...
            ConversionResult conversion = ChiselingConversionHelper.convert(stack.getCount(), stackWorth, target, availableSpace + targetItem.getDefaultMaxStackSize());
            boolean canConvertEntireStack = conversion.leftover() <= 0;
            if(!canConvertEntireStack)
                conversion = ChiselingConversionHelper.convert(stack.getCount(), stackWorth, target, availableSpace + targetItem.getDefaultMaxStackSize());
            if(conversion.numberOfConversions() <= 0)
                continue;
    ...
    public void clicked(int slotIndex, int inventoryIndex, ClickType clickType, Player player){
        ...
        if (clickType == ClickType.SWAP && slotIndex == 0 && (inventoryIndex >= 0 && inventoryIndex < 9 || inventoryIndex == 40)) {
            ...
            if(!currentStack.isEmpty() && (currentStack.getCount() > currentStack.getMaxStackSize() || currentStack.getCount() > inventory.getMaxStackSize(currentStack))){
                ...
                if(!inventoryStack.isEmpty() && !ItemStack.isSameItemSameComponents(currentStack, inventoryStack))
                    return;
                int maxSize = Math.min(currentStack.getMaxStackSize(), inventory.getMaxStackSize(currentStack));
