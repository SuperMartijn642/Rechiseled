...
public abstract class BaseChiselingContainer extends BaseContainer {
    ...
    protected void findRecipe(){
        ...
        if(recipe != null && this.currentRecipe == recipe && this.currentRecipe.contains(item) && this.currentEntry.contains(item)
            && (this.connecting ? this.currentEntry.getConnectingItem(this.shape).equals(item) : this.currentEntry.getRegularItem(this.shape).equals(item)))
            return;
            ...
            for(ChiselingEntry entry : this.currentRecipe.entries()){
                if(entry.contains(item)){
                    for(ChiselingBlockShape shape : ChiselingBlockShape.values()){
                        if(this.connecting && entry.hasConnectingItem(shape) && entry.getConnectingItem(shape).equals(item)){
                            this.currentEntry = entry;
                            this.shape = shape;
                            return;
                        }else if(entry.hasRegularItem(shape) && entry.getRegularItem(shape).equals(item)){
                            this.currentEntry = entry;
                            this.shape = shape;
                            this.connecting = false;
                            return;
                        }else if(!this.connecting && entry.getConnectingItem(shape).equals(item)){
                            this.currentEntry = entry;
                            this.shape = shape;
    ...
    public void setCurrentEntry(int index, ChiselingBlockShape shape, boolean connecting){
        ...
        ItemWithMeta item = connecting ? entry.getConnectingItem(shape) : entry.getRegularItem(shape);
        //noinspection DataFlowIssue
        this.setCurrentStack(item.toStack(convertedAmount));
        if(leftover > 0){
    ...
    public void chiselAll(boolean includeAllShapes){
        ...
        ItemWithMeta targetItem = this.connecting ? this.currentEntry.getConnectingItem(this.shape) : this.currentEntry.getRegularItem(this.shape);
        ...
        for(int index = 0; index < inventory.getSizeInventory(); index++){
            ...
            for(ChiselingBlockShape shape : ChiselingBlockShape.values()){
                if((stackEntry.hasRegularItem(shape) && stackEntry.getRegularItem(shape).matches(stack))
                    || (stackEntry.hasConnectingItem(shape) && stackEntry.getConnectingItem(shape).matches(stack)))
                    stackShape = shape;
            }
