...
public class BaseChiselingContainerScreen<T extends BaseChiselingContainer> extends BaseContainerWidget<T> {
    ...
    protected void addWidgets(){
        ...
        scissorWidget.setScissorOffset(() -> this.screen.getGuiLeft(), () -> this.screen.getGuiTop());
    ...
    public void renderForeground(int mouseX, int mouseY){
        ...
        if(this.recipe != null && this.chiselAllWidget.isFocused()){
            boolean allShapes = GuiContainer.isShiftKeyDown();
            int items = 0;
        ...
        ScreenUtils.drawCenteredString(this.title, 51, 2, ScreenUtils.ACTIVE_TEXT_COLOR);
        ScreenUtils.drawString(ClientUtils.getPlayer().inventory.getDisplayName(), 50, 150);
    }
    ...
    private void updateRecipe(){
        ...
        if(this.recipe != null){
            ItemStack currentItem = this.container.getCurrentStack();
    ...
    private boolean doesItemMatchSearch(ItemWithMeta item){
        ...
        if(isModSearch){
            if(formattedSearchText.length() == 1)
                return true;
            ResourceLocation identifier = Registries.ITEMS.getIdentifier(item.item());
            if(identifier.getResourceDomain().toLowerCase().startsWith(formattedSearchText.substring(1)))
                return true;
            ModContainer mod = Loader.instance().getIndexedModList().get(identifier.getResourceDomain());
            String modName = mod == null ? null : mod.getName();
    ...
    private void chiselAll(){
        Rechiseled.CHANNEL.sendToServer(new PacketChiselAll(GuiContainer.isShiftKeyDown()));
    }
