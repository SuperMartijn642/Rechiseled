...
public class FilterOptionsWidget extends BaseWidget {
    ...
    public static final ResourceLocation BUTTONS = Rechiseled.identifier("screen/filter_buttons");
    public static final ResourceLocation MARKER = Rechiseled.identifier("screen/filter_marker");
    public static final ResourceLocation CHECKMARK = Rechiseled.identifier("screen/checkmark");
    ...
    public void renderBackground(WidgetRenderContext context, GuiGraphicsHelper graphics, int mouseX, int mouseY){
        ...
        graphics.submitSprite(BUTTONS, this.x, this.y, this.collapsedWidth, this.collapsedHeight, p -> p.uv(0, (this.active ? this.isFocused() || this.dropdown.expanded ? 1 : 0 : 2) / 3f, 1, 1 / 3f));
    }
    ...
    private class Dropdown extends BaseWidget {
        ...
        public void renderForeground(WidgetRenderContext context, GuiGraphicsHelper graphics, int mouseX, int mouseY){
            if(!this.expanded)
                return;
            graphics.submitRectangle(this.x, this.y, this.width, this.height, p -> p.color(139, 139, 139));
            int hoveredLine;
            if(this.expanded && mouseX >= this.x && mouseX < this.x + this.width && mouseY >= this.y && mouseY < this.y + this.height){
                ...
                graphics.submitRectangle(this.x, this.y + 1 + 11 * hoveredLine, this.width, 11, p -> p.color(150, 150, 150));
            }else
                hoveredLine = -1;
            this.renderLine(graphics, 0, hoveredLine, FilterOptionsWidget.this.showBlocks, this.showBlocksText);
            this.renderLine(graphics, 1, hoveredLine, FilterOptionsWidget.this.showStairs, this.showStairsText);
            this.renderLine(graphics, 2, hoveredLine, FilterOptionsWidget.this.showSlabs, this.showSlabsText);
            this.renderLine(graphics, 3, hoveredLine, FilterOptionsWidget.this.showNonConnecting, this.showNonConnectingText);
        }

        private void renderLine(GuiGraphicsHelper graphics, int line, int hoveredLine, BooleanSupplier isEnabled, Component text){
            if(isEnabled.getAsBoolean())
                graphics.submitSprite(CHECKMARK, this.x + 3, this.y + 2 + line * 11, 8, 8);
            graphics.submitText(
