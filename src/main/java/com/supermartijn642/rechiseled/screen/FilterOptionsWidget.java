...
public class FilterOptionsWidget extends BaseWidget {
    ...
    private class Dropdown extends BaseWidget {
        ...
        public Dropdown(int x, int y){
            super(x, y, 0, 0);
            int width = Minecraft.getInstance().font.width(TextComponents.fromTextComponent(this.showBlocksText).format());
            width = Math.max(width, Minecraft.getInstance().font.width(TextComponents.fromTextComponent(this.showStairsText).format()));
            width = Math.max(width, Minecraft.getInstance().font.width(TextComponents.fromTextComponent(this.showSlabsText).format()));
            width = Math.max(width, Minecraft.getInstance().font.width(TextComponents.fromTextComponent(this.showNonConnectingText).format()));
