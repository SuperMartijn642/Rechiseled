...
public class RechiseledModel implements IModelGeometry<RechiseledModel> {
    ...
    public IBakedModel bake(IModelConfiguration owner, ModelBakery bakery, Function<RenderMaterial,TextureAtlasSprite> spriteGetter, IModelTransform modelTransform, ItemOverrideList overrides, ResourceLocation modelLocation){
        ...
        for(BlockPart part : elements){
            for(Direction direction : part.faces.keySet()){
                ...
                Pair<RenderMaterial,Boolean> texture = this.getTexture(face.texture, modelGetter);
    ...
    public Collection<RenderMaterial> getTextures(IModelConfiguration owner, Function<ResourceLocation,IUnbakedModel> modelGetter, Set<Pair<String,String>> missingTextureErrors){
        ...
        RenderMaterial particles = this.getTexture("particle", modelGetter).getFirst();
        ...
        for(BlockPart part : this.getElements(modelGetter)){
            for(BlockPartFace face : part.faces.values()){
                RenderMaterial texture = this.getTexture(face.texture, modelGetter).getFirst();
    ...
    private Pair<RenderMaterial,Boolean> getTexture(String texture, Function<ResourceLocation,IUnbakedModel> modelGetter){
        ...
        while(true){
            Either<Pair<RenderMaterial,Boolean>,String> either = this.findTextureEntry(texture, modelGetter);
            Optional<Pair<RenderMaterial,Boolean>> optional = either.left();
            ...
            if(list.contains(texture)){
                ...
                return Pair.of(new RenderMaterial(AtlasTexture.LOCATION_BLOCKS, MissingTextureSprite.getLocation()), false);
            }
    ...
    private Either<Pair<RenderMaterial,Boolean>,String> findTextureEntry(String texture, Function<ResourceLocation,IUnbakedModel> modelGetter){
        ...
        if(parent instanceof BlockModel){
            if(((BlockModel)parent).customData.hasCustomGeometry() && ((BlockModel)parent).customData.getCustomGeometry() instanceof RechiseledModel)
                ...
                for(BlockModel blockmodel = (BlockModel)parent; blockmodel != null; blockmodel = blockmodel.parent){
                    Either<RenderMaterial,String> either = blockmodel.textureMap.get(texture);
                    if(either != null)
        ...
        return Either.left(Pair.of(new RenderMaterial(AtlasTexture.LOCATION_BLOCKS, MissingTextureSprite.getLocation()), false));
    }
