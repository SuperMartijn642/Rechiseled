...
public class RechiseledChiselingRecipes {
    ...
    private static void regularBlockOnly(Identifier recipe, Supplier<ItemLike> block){
        REGISTRATION.chiselingEntry(recipe, entry -> entry.regularBlock(block.get()));
    }

    private static void regularSet(Identifier recipe, Supplier<ItemLike> block, Supplier<ItemLike> stairs, Supplier<ItemLike> slab){
