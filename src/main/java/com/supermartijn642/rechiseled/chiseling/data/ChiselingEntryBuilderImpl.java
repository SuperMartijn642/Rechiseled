...
public class ChiselingEntryBuilderImpl implements ChiselingEntryBuilder {
    ...
    @Override
    public ChiselingEntryBuilder regularItem(ChiselingBlockShape shape, IItemProvider item, float worth){
        if(item == null)
    ...
    @Override
    public ChiselingEntryBuilder regularItem(ChiselingBlockShape shape, IItemProvider item){
        return this.regularItem(shape, item, 1);
    }

    @Override
    public ChiselingEntryBuilder connectingItem(ChiselingBlockShape shape, IItemProvider item, float worth){
        if(item == null)
            throw new IllegalArgumentException("Item must not be null!");
        if(worth <= 0)
            throw new IllegalArgumentException("Worth must be greater than zero!");
    ...
    @Override
    public ChiselingEntryBuilder connectingItem(ChiselingBlockShape shape, IItemProvider item){
        return this.connectingItem(shape, item, 1);
    }

    @Override
    public ChiselingEntryBuilder regularBlock(IItemProvider item, float worth){
        return this.regularItem(ChiselingBlockShape.BLOCK, item, worth);
    }

    @Override
    public ChiselingEntryBuilder regularBlock(IItemProvider item){
        return this.regularBlock(item, 1);
    }

    @Override
    public ChiselingEntryBuilder regularStairs(IItemProvider item, float worth){
        return this.regularItem(ChiselingBlockShape.STAIRS, item, worth);
    }

    @Override
    public ChiselingEntryBuilder regularStairs(IItemProvider item){
        return this.regularStairs(item, 1);
    }

    @Override
    public ChiselingEntryBuilder regularSlab(IItemProvider item, float worth){
        return this.regularItem(ChiselingBlockShape.SLAB, item, worth);
    }

    @Override
    public ChiselingEntryBuilder regularSlab(IItemProvider item){
        return this.regularSlab(item, 0.5f);
    }

    @Override
    public ChiselingEntryBuilder connectingBlock(IItemProvider item, float worth){
        return this.connectingItem(ChiselingBlockShape.BLOCK, item, worth);
    }

    @Override
    public ChiselingEntryBuilder connectingBlock(IItemProvider item){
        return this.connectingBlock(item, 1);
    }

    @Override
    public ChiselingEntryBuilder connectingStairs(IItemProvider item, float worth){
        return this.connectingItem(ChiselingBlockShape.STAIRS, item, worth);
    }

    @Override
    public ChiselingEntryBuilder connectingStairs(IItemProvider item){
        return this.connectingStairs(item, 1);
    }

    @Override
    public ChiselingEntryBuilder connectingSlab(IItemProvider item, float worth){
        return this.connectingItem(ChiselingBlockShape.SLAB, item, worth);
    }

    @Override
    public ChiselingEntryBuilder connectingSlab(IItemProvider item){
