...
public class ChiselingRecipeManagerImpl implements ChiselingRecipeManager {
    ...
    private static final Map<Identifier,PluginEntry> PLUGINS_BY_IDENTIFIER = new HashMap<>();
    private static boolean finalized = false;

    public synchronized static void registerPlugin(Identifier identifier, ChiselingRecipePlugin plugin, int priority){
        if(finalized)
    ...
    private static void loadAnnotationPlugins(){
        ...
        for(ModFileScanData scanData : ModList.get().getAllScanData()){
            ...
            if(scanData.getIModInfoData().isEmpty() || scanData.getIModInfoData().getFirst().getMods().isEmpty())
                continue;
            String modid = scanData.getIModInfoData().getFirst().getMods().getFirst().getModId();
            // Find annotations
            for(ModFileScanData.AnnotationData annotation : scanData.getAnnotations()){
                ...
                try{
                    ...
                    String identifier = (String)annotation.annotationData().getOrDefault("identifier", "main");
                    ...
                    // Add the plugin
                    registerPlugin(Identifier.fromNamespaceAndPath(modid, identifier), plugin, priority);
                }catch(Exception e){
    ...
    public static void loadRecipes(){
        ...
        Holder<Identifier> activePlugin = new Holder<>() {
            @Override
            public Identifier get(){
                Identifier identifier = super.get();
                if(identifier == null)
    ...
    private record PluginEntry(Identifier identifier, int priority, ChiselingRecipePlugin plugin) {
    }
}
