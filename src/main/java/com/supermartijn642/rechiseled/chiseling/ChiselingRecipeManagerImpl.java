...
public class ChiselingRecipeManagerImpl implements ChiselingRecipeManager {
    ...
    public static synchronized void finalizePlugins(){
        ...
        for(EntrypointContainer<ChiselingRecipePlugin> entrypoint : entrypoints){
            ResourceLocation identifier = new ResourceLocation(entrypoint.getProvider().getMetadata().getId(), "entrypoint");
            if(PLUGINS_BY_IDENTIFIER.containsKey(identifier)){
                int index = 2;
                while(PLUGINS_BY_IDENTIFIER.containsKey(new ResourceLocation(identifier.getNamespace(), identifier.getPath() + index)))
                    index++;
                identifier = new ResourceLocation(identifier.getNamespace(), identifier.getPath() + index);
            }
